...
test('move', async () => {
  ...
  await client.emitAsync('move', {robotId: linda._id, x: 4.5, y: 4.5}).should.eventually.be.rejected.with.property('error').that.equals('wrong-arguments');
  await client.emitAsync('move', {robotId: linda._id, x: 1, y: 1}).should.eventually.be.rejected.with.property('error').that.equals('already-there');
  await client.emitAsync('move', {robotId: linda._id, x: 1, y: 0}).should.eventually.be.rejected.with.property('error').that.equals('coords-occupied');
